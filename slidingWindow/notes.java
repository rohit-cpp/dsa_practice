...
class Solution {
    public int subarraySum(int[] nums, int k) {
        ...
        // HashMap stores: <Prefix Sum, Frequency of this sum>
        HashMap<Integer, Integer> map = new HashMap<>();
        
        // Base case: We've seen a sum of 0 exactly 1 time (before starting the array)
        ...
        for (int num : nums) {
            ...
            if (map.containsKey(target)) {
                // Add the frequency of how many times we've seen that target sum
                count += map.get(target);
            }
            
            // Add the current prefixSum to the map, or increment its count
