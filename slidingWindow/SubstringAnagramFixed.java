...
// 1. The Algebra (The Math Translation)The problem asks you to check if the average of your current window is greater than or equal to the threshold.Written as a math equation, it looks like this:currentSum / k >= thresholdIn algebra, if you want to get rid of a division, you multiply both sides by the denominator. If we multiply both sides of that equation by k, it becomes:currentSum >= threshold * kBecause the threshold and k never change during the entire program, threshold * k will always be the exact same number. So, we calculate that number once at the very beginning and call it our targetSum.2. A Concrete ExampleLet's use the numbers from the problem: k = 3 and threshold = 4.targetSum = 4 * 3 = 12.Now imagine your loop calculates a window currentSum of 13. Let's look at the two ways to check it:The Old Way (Division): Is 13 / 3 >= 4?The CPU calculates $13 / 3 = 4.33$.Then it asks: Is $4.33 \ge 4$? Yes.The Target Sum Way: Is 13 >= 12?The CPU asks: Is $13 \ge 12$? Yes.Both methods give you the exact same Yes or No answer, but the second one skips the math entirely.3. Why Interviewers Love This (The Performance Boost)Why do we care about changing this if both give the correct answer? CPU Performance.For a computer processor, operations have different costs:Addition/Subtraction: Extremely fast.Multiplication: Very fast.Division: Extremely slow (comparatively).If your array nums has 100,000 elements (which is the constraint in this problem), doing currentSum / k inside your loop means you are forcing the CPU to perform 100,000 slow division operations.By calculating int targetSum = t * k at the very top of your code, you do one fast multiplication, and zero divisions. Inside the loop, you are just comparing two numbers (currentSum >= targetSum), which is practically instantaneous.
